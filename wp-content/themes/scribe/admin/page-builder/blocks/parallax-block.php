...
if(!class_exists('PG_Parallax_Block')) {
	class PG_Parallax_Block extends AQ_Block {
		...
		function __construct() {
			...
			//create the widget
			parent::__construct('pg_parallax_block', $block_options);
	...
	?>
			...
			<p class="empty-column">
				<strong>Drag and Drop blocks below this line.</strong>
			</p>
				<hr>
	...
	<?php
		...
		function form_callback($instance = array()) {
			...
			if(isset($template_id)) {
				...
				<?php 
				echo '<div class="block-settings-column cf" id="block-settings-'.$number.'">';
		?>
			...
			if(isset($template_id)) {
				...
				//open section wrapper
					...
					echo '</div></div></div></div>';
						...
						if ( $detect->isMobile() ) {
echo  '<section class="sc-parallax" style="background-image:url('.$image.');font-size:'.$type.';" >';
						} else {
						
	      echo  '<div class="parallax-holder"><section class="sc-parallax" style="background-image:url('.$image.');font-size:'.$type.';" data-stellar-background-ratio="0.5" >';}
		 
	           echo '<div class="row"><div class="containerCentered">
			<h3>'.$title.'</h3></div> <div class="sc-mask"></div><div class="container">';
