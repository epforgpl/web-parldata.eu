...
if(!class_exists('AQ_Page_Builder')) {
	class AQ_Page_Builder {
		...
		function __construct( $config = array()) {
			...
			$defaults['menu_title'] = __('Page Builder', 'scribe');
			$defaults['page_title'] = __('Page Builder', 'scribe');
			$defaults['page_slug'] = __('aq-page-builder', 'scribe');
		...
		function register_template_post_type() {
			...
			if(!post_type_exists('template')) {
				...
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Aqua Page Builder notice: </strong>'. __('The \"template\" post type already exists, possibly added by the theme or other plugins. Please consult with theme author to consult with this issue', 'scribe') .'</p></div>';"));
			}
		...
		function display_blocks( $template_id ) {
			...
			if(empty($blocks)) {
				...
				echo __('Drag block items from the left into this area to begin building your template.', 'scribe');
				echo '</p>';
				return;
		...
		**/
		function display_template($template_id) {
			...
			if(empty($blocks)) {
				...
				echo '<p class="empty-template">';
				echo __('This template is empty', 'scribe');
				echo '</p>';
		...
		function add_shortcode() {
			...
			if ( !array_key_exists( 'template', $shortcode_tags ) ) {
				...
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Aqua Page Builder notice: </strong>'. __('The \"[template]\" shortcode already exists, possibly added by the theme or other plugins. Please consult with the theme author to consult with this issue', 'scribe') .'</p></div>';"));
			}
		...
		function add_media_button( $button ) {
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				if ( version_compare( $wp_version, '3.5', '<' ) ) {
					$img 	= '<img src="' . AQPB_DIR . '/assets/images/aqua-media-button.png" width="16px" height="16px" alt="' . esc_attr__( 'Add Page Template', 'scribe' )  . '" />';
					$output = '<a href="#TB_inline?width=640&inlineId=aqpb-iframe-container" class="thickbox" title="' . esc_attr__( 'Add Page Template', 'scribe' )  . '">' . $img . '</a>';
				} else {
					...
					$output = '<a href="#TB_inline?width=640&inlineId=aqpb-iframe-container" class="thickbox button" title="' . esc_attr__( 'Add Page Template', 'scribe' ) . '" style="padding-left: .4em;">' . $img . ' ' . esc_attr__( 'Add Template', 'scribe' ) . '</a>';
				}
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				<script type="text/javascript">
					function insertTemplate() {
						...
						if ( '' == id ) {
							alert("<?php echo esc_js( __( 'Please select your template first!', 'scribe' ) ); ?>");
							return;
						}
				...
				<div id="aqpb-iframe-container" style="display: none;">
					<div class="wrap" style="padding: 1em">
						...
						if ( empty( $templates ) ) {
							echo sprintf( __( 'You don\'t have any template yet. Let\'s %s create %s one!', 'scribe' ), '<a href="' .admin_url().'themes.php?page=aq-page-builder">', '</a>' );
							return;
						}
						?>						

						<h3><?php _e( 'Choose Your Page Template', 'scribe' ); ?></h3><br />
						...
						<input type="button" id="aqpb-insert-template" class="button-primary" value="<?php echo esc_attr__( 'Insert Template', 'scribe' ); ?>" onclick="insertTemplate();" />
						<a id="aqpb-cancel-template" class="button-secondary" onclick="tb_remove();" title="<?php echo esc_attr__( 'Cancel', 'scribe' ); ?>"><?php echo esc_attr__( 'Cancel', 'scribe' ); ?></a>
		...
		function contextual_help() {
			...
			if($screen->id == $this->page) {
				...
				$screen->set_help_sidebar(
					'<p><strong>' . __('For more information:','scribe') . '</strong></p>' .
					'<p>' . __('<a href="http://pixelgrapes.com/docs/scribe" target="_blank">Documentation</a>') . '</p>'
				);
				
				// Main overview tab
				$screen->add_help_tab( array(
				'id'		=> 'overview',
				'title'		=> __('Overview','scribe'),
