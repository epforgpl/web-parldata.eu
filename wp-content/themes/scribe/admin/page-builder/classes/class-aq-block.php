...
if(!class_exists('AQ_Block')) {
	class AQ_Block {
	 	...
	 	function block($instance) {
	 		extract($instance);
	 		echo __('function AQ_Block::block should not be accessed directly. Output generated by the ', 'scribe') . strtoupper($id_base). ' Class';
	 	}
	 	...
	 	function form($instance) {
	 		echo '<p class="no-options-block">' . __('There are no options for this block.', 'scribe') . '</p>';
	 		return 'noform';
	 	}
	 	...
	 	function before_block($instance) {
	 		...
	 		$class = '';
			if($id_base == 'pg_features_block') {
				 			$class = ' col-md-' . $intsize . ' col-sm-6 col-xs-12';
				 		}
				 		elseif ($id_base == 'pg_team_block') {
								 			$class = ' col-md-' . $intsize . ' col-sm-4 col-xs-12';
								 		}
								 		elseif ($id_base == 'pg_stats_block') {
												 			$class = ' col-md-' . $intsize . ' col-sm-4 col-xs-12';
												 		}
												 		else {
	 			$class = ' col-md-' . $intsize . ' col-xs-12';
	 		}
	 		echo '<div id="aq-block-'.$template_id.'-'.$number.'" class="aq-block aq-block-'.$id_base. $class . ' clearfix">';
	 	}
